...
def build_model(args,device, path=None):
    ...
    if args.network == 'R18':
        if args.dataset == 'CUB-200':
            model = PretrainedResNet(num_classes=args.num_classes).to(device)
        else:
...
def main(args=None):
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        if(epoch<=args.start_correct):        
            ...
            selected_examples, selected_examples_labels = reliable_pseudolabel_selection(args, device, train_loader, features, epoch)
        elif(epoch > args.start_correct):
            print('######## Pseudo-labelling ########')
            selected_examples, selected_examples_labels = reliable_pseudolabel_selection(args, device, train_loader, features, epoch, model_preds)
